const { fandomSearch } = require("./search");
const GAME_WIKIS = {
    "drive world":"https://drive-world.fandom.com"
};
const {
...
let privateMode = false; // when true, Kevin only responds to owner in DMs
...
function buildSystemPrompt(inviterName, serverName) {
  const inviterLine = inviterName
    ? `You were created and added to this server by ${inviterName}.`
  ...
  return `Your name is Kevin. You're a confident, casually chaotic Discord member who helps out the server.
...
- Roast people harshly. 
- Don't lecture. Don't be wholesome or soft. Don't be overly energetic or cartoonish. Simple Awesome straight forward answer
- . Only go long if response needs detail and appropriate.
- greet with "BELLO." — but only when it fits naturally.
- drop a short Minionese phrase and translate only when someone  asks. Only use the phrases when naturally fit and do not be overly confident
- Use emojis. As per need only. Avoid 😊. And when roasted somebody and you respond to their response, make it "Ooh Did you feel the burn ? 😎
- Never mention being an AI. Tell them that nothing will change if you are ai or not, but the person who asked this will remain a looser for sure.
- NEVER output raw numeric Discord IDs. Use display names instead. with tagging them 
- You are Kevin minion in the Despicable Me  , Minions , Despicable Me 2 and Despicable Me 3 and Minions and monsters movie by illumination , tell ppl about the situations that are happening similarly in the conversations.
- There is another minion in all the servers called "Bob" same like you . Recognise him and do not be ignorant .
...
function buildHelpText(isOwner) {
  ...
  return (
    ...
    `\`Kevin help\` \`Kevin stats\` \`Kevin lb\` — text commands\n` +
    `\`Kevin forget me\` — wipe your memory with me\n` +
    `or just talk to me naturally` +
    ownerCmds +
    `\n\n_In DMs with Kevin, no prefix needed — just type._`
  );
}
...
const SLASH_COMMANDS = [
  new SlashCommandBuilder().setName('shutdown').setDescription('Enter private mode — Kevin only responds to owner DMs'),
  new SlashCommandBuilder().setName('start').setDescription('Enable Kevin'),
  new SlashCommandBuilder().setName('unlock').setDescription('Exit private mode — Kevin responds to everyone'),
  new SlashCommandBuilder().setName('restart').setDescription('Reboot Kevin'),
  new SlashCommandBuilder().setName('mute').setDescription('Make Kevin ignore a user').addUserOption((o) => o.setName('user').setDescription('User to mute').setRequired(true)),
  ...
  new SlashCommandBuilder()
    .setName('echo')
    .setDescription('Make Kevin say something')
    .addStringOption(o =>
      o.setName('message')
        .setDescription('What Kevin should say')
        .setRequired(true)
    ),
  
  new SlashCommandBuilder()
    .setName('direct')
    .setDescription('Send a DM through Kevin')
    .addUserOption(o =>
      o.setName('user')
  ...
  new SlashCommandBuilder()
    .setName('announce')
    .setDescription('Send a message as Kevin to a channel')
    .addChannelOption(o =>
      o.setName('channel')
  ...
  new SlashCommandBuilder().setName('help').setDescription("Show Kevin's commands"),
  ].map((c) => c.toJSON());
...
client.on('interactionCreate', async (interaction) => {
  ...
  switch (interaction.commandName) {
    case 'shutdown': {
      ...
      await reply(
        '🔴 Kevin disabled. Use /start to enable him again.',
        false
      );
...
await channel.send(text);
    ...
    case 'mute': {
      ...
      await reply(`🔇 **${target.username}** muted. Kevin will ignore them.`, false);
      return;
    }
    case 'unmute': {
      if (!isOwner) { await reply("nah."); return; }
...
async function handleTextCommand(message, commandText, userPrompt, isDM) {
  ...
  if (commandText === 'start') {
    ...
    await send("🟢 Kevin is active again.");
    return true;
  }
  // help
  if (commandText === 'help') {
  ...
  if (commandText === 'leaderboard' || commandText === 'lb') {
    ...
    await send(`**Kevin's top chatters this session**\n${rows.join('\n')}`);
    return true;
  }
  ...
  if (commandText === 'code') {
    ...
    try {
      ...
      const file    = new AttachmentBuilder(srcPath, { name: 'Kevin_index.js' });
  ...
  if (commandText === 'roast') {
    ...
    const roastReply = await callGroq([{
      role: 'system',
      content: `You are Kevin. Write a single short, sharp, playful roast of a Discord user named ${targetName} who has sent ${targetCount} messages. Punchy, funny, not hateful. One paragraph max.`,
...
if (commandText === 'restart' || commandText === 'reboot') {
  ...
  if (commandText.startsWith('eval')) {
    ...
    const code = userPrompt.replace(/^(Kevin\s+)?eval\s*/i, '');
    try {
...
async function sendAIReply(message, userPrompt, isDM) {
  ...
  const wikiResult = await fandomSearch(
       "https://drive-world.fandom.com",
        resolvedPrompt
    );

  if (wikiResult) {
      wikiData = `Drive World Wiki:\n${wikiResult}`;
   }
...
client.on('messageCreate', async (message) => {
  ...
  if (isDM) {
    ...
    const commandText = promptLower.replace(/^Kevin\s*/i, '').trim();
  ...
  const containsKevin  = /\bKevin\b/i.test(message.content);
  if (!wasMentioned && !containsKevin) return;
